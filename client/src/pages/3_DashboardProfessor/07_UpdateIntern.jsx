...
export default function UpdateIntern() {
  ...
  const getIntern = () => {
    ...
    axios.get(`${host}/api/internship/${id}`, config
      ...
      setIntern({...response.data, pay: response.data.stipend===0?"UNPAID":"PAID"})
  ...
  const handleSubmit = async (e) => {
    ...
    const { title, field, description, expected_skills, application_deadline, start_date, end_date, no_of_offers, stipend, type, min_cgpa } = intern;
    ...
    if (intern.pay==="UNPAID"){finalData.stipend=0;}
    if (min_cgpa!==""){finalData.min_cgpa=+min_cgpa;}
    setLoading(true);
    axios.put(
      `${host}/api/internship/update/`,
      finalData,
      config
    ).then(function () {
      setLoading(false);
      alert("Internship Updated Successfully");
      navigate(`/professor/internship/${id}`);
    }).catch(function (error) {
      setLoading(false);
      if (error.response.status === 401) {
        alert("Session Expired. Please login again.");
                ...
                <form className="my-2 d-flex flex-column align-content-between">
                  ...
                  <div>
                    <div className="form-check" style={{ fontSize: "20px" }}>
                      <input className="form-check-input" name="type" type="radio" value="REMOTE" onChange={handleChange} checked={intern.type==="REMOTE"} />
                      <small className="form-check-label" htmlFor="defaultCheck1">Remote (Online)</small>
                    </div>
                    <div className="form-check" style={{ fontSize: "20px" }}>
                      <input className="form-check-input" name="type" type="radio" value="ONSITE" onChange={handleChange} checked={intern.type==="ONSITE"} />
                      <small className="form-check-label" htmlFor="checkbox">On-site (Offline at the campus of your institute)</small>
                    </div>
                    <div className="form-check" style={{ fontSize: "20px" }}>
                      <input className="form-check-input" name="type" type="radio" value="HYBRID" onChange={handleChange} checked={intern.type==="HYBRID"} />
            ...
            <div className="w-75">
              ...
              <div className="container my-5 mx-5">
                <form className="my-2 d-flex flex-column align-content-between">
                  ...
                  <div>
                    <div className="form-check" style={{ fontSize: "20px" }}>
                      <input className="form-check-input" name="pay" type="radio" value="PAID" onChange={handleChange} checked={intern.pay==="PAID"} />
                      <small className="form-check-label" htmlFor="defaultCheck1" >Paid</small>
                    </div>
                    <div className="form-check" style={{ fontSize: "20px" }} >
                      <input className="form-check-input" name="pay" type="radio" value="UNPAID" onChange={handleChange} checked={intern.pay==="UNPAID"} />
                      <small className="form-check-label" htmlFor="checkbox">Unpaid</small>
                    </div>
                  </div>
                  {intern.pay === "PAID" && (
                    <TextArea onChange={handleChange} value={intern.stipend} name="stipend" label="Total Amount as Stipend (in INR)" widthp="70%" placeholder='Eg: 50000' />
                  )}
                  <TextArea onChange={handleChange} value={intern.min_cgpa} name="min_cgpa" label="Minimum CGPA Required to Apply (Enter a decimal value between 0 and 10)" widthp="30%" placeholder='7.000' />
